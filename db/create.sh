...
EXIST_QUERY="SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'testtable');"
EXIST_RES=$(psql -h db -p 5432 -U ${POSTGRES_USER} -c "$EXIST_QUERY" $dbname)
EXIST_RES=${EXIST_RES/exists/""}
echo $EXIST_RES

echo -e "1" > /data/script_data/save.csv
...
if [[ "$EXIST_RES" == *"t"* ]]; then
    ...
    psql -h db -p 5432 -U $POSTGRES_USER -af load.sql $dbname
else
    echo "UNKNOWN ERROR."
    exit 1
fi
